...
export const generatewebsite = async (req, res) => {
  try {
    ...
    if (user.credits < 50) {
      return res.status(403).json({ message: "Not enough credits to generate website" });
    }
    ...
    if (!parsed.code) {
      return res.status(500).json({ message: "Failed to generate valid response after multiple attempts" }, raw);
    }
    ...
    return res.status(200).json({ message: "Website generated successfully", websiteId: website._id, remainingCredits: user.credits });
...
export const changes = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `Error updating website: ${error.message}` });
  }
}
...
export const getAll= async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `getAll website error: ${error.message}` });
  }
}
...
export const deploy = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Website deployed successfully", deployUrl: website.deployUrl });
...
export const getBySlug = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `Error fetching website by slug: ${error.message}` });
  }
}
